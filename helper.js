import {createNativeStackNavigator} from '@react-navigation/native-stack';
import {VStack, HStack, Alert, IconButton, CloseIcon,Text} from 'native-base';

const Stack = createNativeStackNavigator();

function getPostDateHours(publishDate) {
  const publishedAt = new Date(publishDate);
  const now = new Date();

  const diffInMs = now - publishedAt;
  const diffInMinutes = Math.floor(diffInMs / 1000 / 60);
  const diffInHours = Math.floor(diffInMinutes / 60);
  const diffInDays = Math.floor(diffInHours / 24);

  if (diffInMinutes < 60) {
    return `${diffInMinutes} mins ago`;
  } else if (diffInHours < 24) {
    return `${diffInHours} hrs ago`;
  } else {
    const remainingHours = diffInHours % 24;
    return `${diffInDays} day${diffInDays > 1 ? 's' : ''} ${remainingHours} hr${
      remainingHours > 1 ? 's' : ''
    } ago`;
  }
}
function AlertPopUp() {
  const statusArray = [
    {
      status: 'success',
      title: 'Selection successfully moved!',
    },
    {
      status: 'error',
      title: 'Please try again later!',
    },
    {
      status: 'info',
      title: 'We are going live in July!',
    },
    {
      status: 'warning',
      title: 'Poor internet connection.',
    },
  ];
  return (
    <Stack space={3} w="100%" maxW="400">
      {statusArray.map(status => {
        return (
          <Alert w="100%" status={status.status}>
            <VStack space={2} flexShrink={1} w="100%">
              <HStack flexShrink={1} space={2} justifyContent="space-between">
                <HStack space={2} flexShrink={1}>
                  <Alert.Icon mt="1" />
                  <Text fontSize="md" color="coolGray.800">
                    {status.title}
                  </Text>
                </HStack>
                <IconButton
                  variant="unstyled"
                  _focus={{
                    borderWidth: 0,
                  }}
                  
                  icon={<CloseIcon size="3" />}
                  _icon={{
                    color: 'coolGray.600',
                  }}
                />
              </HStack>
            </VStack>
          </Alert>
        );
      })}
    </Stack>
  );
}
export {Stack, getPostDateHours, AlertPopUp};
